...
def forward_step(
    ...
    # calculate losses
    pitch_loss = F.cross_entropy(pred_pitch, pitch)
    velocity_loss = F.mse_loss(pred_velocity, velocity)
    dstart_loss = F.mse_loss(pred_dstart, dstart)
    duration_loss = F.mse_loss(pred_duration, duration)
    ...
    pitch_acc = (torch.argmax(pred_pitch, dim=1) == pitch).float().mean()
    velocity_r2 = M.r2_score(pred_velocity, velocity)
    dstart_r2 = M.r2_score(pred_dstart, dstart)
...
def validation_epoch(
    ...
    for batch_idx, batch in val_loop:
        ...
        _, metrics = forward_step(model, batch, loss_lambdas, device)
...
def train(cfg: OmegaConf):
    ...
    for epoch in range(cfg.train.num_epochs):
        ...
        for batch_idx, batch in train_loop:
            # metrics returns loss and additional metrics if specified in step function
            loss, metrics = forward_step(model, batch, loss_lambdas, device)
